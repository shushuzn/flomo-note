...
模板前缀回显（2026-09-26 新增 ERR）：「结论先行」等 SKILL 条目名/格式指令词写进正文即判错——结论直接作为正文首段第一句，不加引导词。
...
def check(content):
    ...
    # 5.7) 模板前缀回显检测（ERR）——2026-09-26 事故加固
    # 事故：三张概念卡正文全部写了「结论先行：」前缀——这是 SKILL H14 的条目名（写给执行者的
    # 格式指令），被原样回显成卡片内容；写云后才被用户发现。此前脚本层只有占位/载体/来源行
    # 检测，对「模板指令词回显」完全无覆盖，故补本项。
    #
    # 两层规则（从严设计，防误伤）：
