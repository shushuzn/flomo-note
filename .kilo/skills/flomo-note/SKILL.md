...
- 一事一卡：一条 memo 一个念头/一条摘录/一个观点/一条事实，简短独立。不建本地文件夹，不做双链。
- 纯标签聚合、不建索引卡：笔记组织只靠 #标签 层级（多级 / 细分），不建立任何索引卡 / MOC / 母题目录；顶层直接用领域或主题名，需要找某主题时靠标签检索，而非额外的索引卡导航。此定位为项目贯例，不因卡量增长而改变。
...
- 云端状态与机制均不在记忆复述：云端标签/卡的全量清单、总数、以及已写入 SKILL 的规则/机制（双卡模板、跨顶层 MOC、口诀化、捕捉持久线索等）全部不进 project_memory——云端状态需时现查 flomo.tag_tree，机制唯一源为 SKILL.md。记忆只保留 SKILL 与云端都重建不了的增量：观察项待办（设触发条件、悬置即删）、未并入 SKILL 的项目特有经验。
