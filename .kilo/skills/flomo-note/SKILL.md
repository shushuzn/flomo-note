...
7. 复盘建议（收口后必做）：结合当轮所写 memo，按多层面核对清单逐层过一遍，提出 ≥1 条可执行改进建议，随收口交用户；须具体可落地（如"新增某顶层主标签""合并某簇标签""某标签已厚可建索引"），说明触发理由，不得空泛。核对清单（每层都要说明"是否改动 + 理由"，无改动也明示）：
  - SKILL.md：执行细则是否需要补充/修正（如新踩的坑、参数、规范）。
  - AGENTS.md：总则是否触及（全局红线、目录分工），一般不动，说明即可。
  - 云端既有 memo：是否有旧卡需更新/删除/合并（过时、重复、部分重叠）。
  - 云端标签体系 + MOC/索引：顶层结构调整、某领域卡已厚可建索引卡（MOC）、跨领域同义标签观察。
  - 脚本/工具：是否有可自动化的静态自检（写入前校验 JSON/tags/格式）、可复用调用封装。
  - 记忆/惯例：是否需沉淀进 user_profile / project_memory 的长期偏好。
  - 创新：本轮/近期是否值得沉淀新东西——如新概念卡、新索引/MOC、新提炼模板或新流程机制；这是常规复盘之上主动"增值"的一层，无则明示"本轮无新增方向"。
