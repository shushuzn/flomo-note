...
- 正文内防标签误识别：flomo 扫全文 #xxx 当标签非仅首行。提及 issue/编号/PR/章节须去 # 改写（如"issue 8435"）；档位/排名里的 #数字（如"≤#150""#1 档"）也会整段吞入标签，一律改用中文（"第150位""榜首"）；写后发现误识别用 memo_update 覆盖正文去掉 # 即可，脏标签无引用即消失。
