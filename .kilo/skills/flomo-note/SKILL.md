...
- 状态只记指针与增量：云端状态记顶层标签总数与清单即可，不追踪卡总数（需要时查云端），不逐层复述每张卡的一句话明细；某层卡数达阈值或因其他原因值得单独记录时才补记该层。
- 机制不在记忆复述：凡已写入 SKILL 的规则/机制（双卡模板、跨顶层 MOC、口诀化、捕捉持久线索等），project_memory 只记"机制见 SKILL.md"，不复制正文——SKILL.md 是唯一细则源。
