...
6. **写云（新建，需授权）**：把最终正文 + 首行标签段**完整展示给用户**，说明将写入你的 flomo 账号；经同意后调 `flomo.memo_create`（正文=memo 内容，标签=首行标签段，内容与展示一致）。除非已明确约定"写卡直接落云"，否则**必须停下等同意**；本次不写云须明示。（若步骤 4 查重判定为**更新**而非新建，则走第 7 步 `memo_update`。）
7. **更新云端 memo（需授权）**——触发：① 查重判定"部分重叠"，由第 6 步建卡改为更新原 memo；② 用户点名修改某条（改正文 / 补标签 / 改来源 / 去重）。做法：先用 `flomo.memo_search` + `flomo.memo_batch_get` 定位并通读原 memo → 明确要改动的那一点 → 向用户展示**改动前后** → 获同意后调 `flomo.memo_update`（同样保持首行标签段）。`memo_update` 是**写操作需授权**，绝不静默改云端；改前必须让你看清拟改动内容。
8. **复盘建议（收口后必做）**：结合当轮所写 memo，提出 ≥1 条对项目（云端标签体系、写法、方向）的可执行改进建议，随收口交用户；须具体可落地（如"新增某顶层主标签""合并某簇标签""某标签已厚可建索引"），说明触发理由，不得空泛。
