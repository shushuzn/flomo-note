...
7. 复盘建议（收口后必做）：结合当轮所写 memo，输出两条建议——① 常规建议：从多层面（SKILL / AGENTS / 既有 memo / 标签+MOC / 脚本工具 / 记忆惯例）中找出最值得的一条可执行改进建议，不逐层罗列"有没有改动"；② 创新建议：必须给出 ≥1 条主动增值方向（新概念卡、新索引/MOC、新提炼模板、新流程机制），不得以"本轮无新增方向"敷衍。两条都须具体可落地（如"新增某顶层主标签""合并某簇标签""某标签已厚可建索引""沉淀新概念卡"），说明触发理由，不得空泛。
