...
6. 写云（新建，需授权）：把最终正文 + 首行标签段完整展示给用户，说明将写入你的 flomo 账号；经同意后调 flomo.memo_create（正文=memo 内容，标签=首行标签段，内容与展示一致）。写入前硬校验：带来源 URL 的 memo，其来源必须已实际 WebFetch/抓取成功并基于真实内容写作（未抓取/抓取失败/凭链接猜测 → 禁止写云，先补抓取）；无来源的纯想法 memo 不受此限。除非已明确约定"写卡直接落云"，否则必须停下等同意；本次不写云须明示。若第 4 步查重判定为"部分重叠"需更新，则本步不新建，计入第 8 步统一更新。
7. 复盘建议（收口后必做）：结合当轮所写 memo，输出两条建议——① 常规建议：从多层面（SKILL / AGENTS / 既有 memo / 标签+MOC / 脚本工具 / 记忆惯例）中找出最值得的一条可执行改进建议，不逐层罗列"有没有改动"；② 创新建议：必须给出 ≥1 条主动增值方向（新概念卡、新索引/MOC、新提炼模板、新流程机制），不得以"本轮无新增方向"敷衍。两条都须具体可落地（如"新增某顶层主标签""合并某簇标签""某标签已厚可建索引""沉淀新概念卡"），说明触发理由，不得空泛。对 SKILL 文档本身的改动遵循"更新/合并/删除三态"：更新=改表意不换原意或加新行；合并=两条内容重复或过近则合并为一条（合并后语义不丢）；删除=某条已过时/已被取代/不再适用可以明确判定时删行（删除必须给出判定理由，不得为删而删）。复盘建议若牵动 SKILL 改动，先向用户说明将做"更新/合并/删除"中哪一种、改动哪一行、理由为何，获同意后落地；不改动则明示"本轮 SKILL 不改动"。每条建议（不论常规/创新）都须主动明确提示用户其执行方式并征询是否落地，不得只陈述建议不询问——用户说"主动提及"即指这一点；已被用户选过/正在做的事要标"已覆盖"或"已落地"，尚未拍板的创新方向要标"待你决定"，让用户一眼看清每条建议的实际处置状态。
...
- 跨顶层母题索引卡（MOC 触发条件）：当同一母题横跨 ≥2 个顶层、且各顶层下已有"事实+框架"双卡结构时，建一张单级顶层索引卡（如 #自主知识体系）做跨领域主题导航，用双链串起各顶层相关卡；母题只在单一顶层内时用该顶层的领域索引卡即可，不必建跨顶层索引。
- 方法论类索引卡（跨顶层 MOC 的去重特例）：可复用方法论/分析框架也横跨多个顶层（各顶层多有 /方法论 子标签），且数量渐多时，可单独建一张 `#方法论` 式跨领域索引卡做"分析框架总目录"，双链串起各层框架卡。与领域母题索引卡（如 #自主知识体系）的去重原则：框架卡一律挂 `#方法论` 总索引承载"方法归类"；领域母题索引卡负责"领域归属"导航。两者可重叠指向同一张框架卡而不互相排挤（一张框架卡既能被 #方法论 收录、又能被某领域索引收录），不因归属重叠而删卡。触发条件：方法论框架卡达 ≥6 张且分布 ≥4 顶层时，考虑建 #方法论 索引；未来新增框架卡挂入该索引。
